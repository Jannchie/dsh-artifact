window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function passthroughCodec(typeSymbol) {
			// DSH 0.1.5's gateway parses through `schema`; 0.1.7's Typert registry
			// rejects a strict codec without a lazy `create()` factory. Carry both.
		...
		const zh = {
			...
			"view.all": "全部工件",
			"view.library": "工件",
			"empty.session": "这个会话还没有工件",
			"empty.sessionHint": "智能体在这个会话里写的工件会出现在这里；其他工件在侧边栏的「工件」里",
			"action.library": "全部工件",
			"list.aria": "工件列表",
			"list.count": "{count} 个工件",
			"empty.none": "还没有工件",
		...
		const en = {
			...
			"view.session": "This session's artifacts",
			"view.all": "All artifacts",
			"view.library": "Artifacts",
			"empty.session": "No artifacts in this session yet",
			"empty.sessionHint": "Artifacts the agent writes in this session appear here; the rest are under Artifacts in the sidebar",
			"action.library": "All artifacts",
		...
		/**
		 * The first of several names the icon set exports. DSH 0.1.7 renamed the
		 * set from pixel sizes (`…Outline16`) to weights (`…OutlineRegular`);
		 * a name that resolves to nothing would render an invalid element and
		 * take the whole view down, so an unknown glyph is null and the button
		 * simply draws without it.
		 */
		function glyph(...names) {
			for (const name of names) if (typeof primitives[name] === "function") return primitives[name];
			return null;
		}
		 ...
		 */
		const Icon = {
			back: glyph("IconChevronLeftOutlineRegular", "IconChevronLeftOutline14"),
			refresh: glyph("IconRefreshOutlineRegular", "IconRefreshOutline16"),
			source: glyph("IconCodeOutlineRegular", "IconCodeOutline16"),
			preview: glyph("IconBrowseOutlineRegular", "IconBrowseOutline16"),
			remove: glyph("IconTrashOutlineRegular", "IconTrashOutline16"),
			library: glyph("IconBrowseOutlineRegular", "IconBrowseOutline16"),
		};
		...
		function createBrowser(remote) {
			return function ArtifactBrowser(props) {
				const t = props.t;
				// The conversation tab passes its session and lists what that
				// session wrote; the library panel passes none and lists everything.
				const sessionScope = props.scope === "session" && props.sessionId ? props.sessionId : null;
				...
				const refresh = react.useCallback(async () => {
					const result = await call(remote, "listArtifacts", sessionScope ? { sessionId: sessionScope } : {});
					if (!result.ok) {
					...
					artifacts.length === 0
						? react.createElement(
								...
								react.createElement(
									...
									t(sessionScope ? "empty.session" : "empty.none"),
								),
								react.createElement(
									"div",
									{ className: "dsh-artifact__emptyhint" },
									t(sessionScope ? "empty.sessionHint" : "empty.hint"),
								),
								sessionScope && props.openLibrary
									? iconButton({ onClick: props.openLibrary }, Icon.library, t("action.library"))
									: null,
								sessionScope || root === ""
									? null
									: react.createElement(
				...
				return react.createElement(
					...
					react.createElement(
						...
						react.createElement(
							"h2",
							{ className: "dsh-artifact__heading" },
							t(sessionScope ? "view.session" : "view.all"),
						),
						react.createElement(
							"span",
		...
		/** Main-panel key and sidebar icon id of the artifact library. */
		const LIBRARY_ID = "artifacts";
		...
		async function apply(ctx) {
			...
			// The library is a global main panel, opened through the layout
			// service. Resolved per click: `layout` is not injected, so it may
			// arrive after this plugin did — and a host without it (0.1.5) simply
			// shows no library button.
			const openLibrary = () => {
				const layout = ctx.get("layout");
				if (layout === undefined || typeof layout.selectPanel !== "function") return;
				layout.selectPanel(LIBRARY_ID);
			};
			const SessionView = (props) =>
				react.createElement(Browser, { ...props, scope: "session", openLibrary });
			const LibraryView = (props) => react.createElement(Browser, { ...props, scope: "global" });
			const LibraryIcon = (props) =>
				Icon.library === null ? null : react.createElement(Icon.library, { size: props.size ?? 18 });

			// Every artifact, as a global page beside Plugins: `main` holds the
			// panel, `sidebar.panellist` its icon, paired by one id. Both seats are
			// declared by the shell from DSH 0.1.7; elsewhere these never mount.
			ctx.slots.inject("main", () =>
				ctx.slots.register({ name: "main", key: LIBRARY_ID, locale: NS }, LibraryView),
			);
			...
			// area while selected. A tab over a conversation is about that
			// conversation, so it lists what this session wrote. `label` is a
			// thunk so the tab text re-resolves on a language switch; `order`
			// places this tab after the shipped two.
