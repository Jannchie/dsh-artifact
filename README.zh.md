...
它会调用 `artifact` 工具，文档出现在该会话的**产物**标签页里，这里只列本会话写的产物。所有会话的产物都在侧边栏「插件」旁的**产物**库里（DSH 0.1.7+）。点进去读，`‹` 返回。

这个版本之前写的产物、以及在 DSH 0.1.7 之前的宿主上写的产物没有会话记录，只出现在产物库里。
...
`path` 相对产物库解析，`.html` 后缀可省，所以 `write path:"q3-report"` 得到 `q3-report.html`。产物库是一层平铺的架子：跑到外面、落进子目录、或者以点开头的路径都会被拒绝，而不是存到浏览器找不到的地方。

版本历史刻意没有做进这个工具。模型刚写完这份文档，它知道自己写了什么；"这版改了什么"是人隔一天才会问的问题，所以答案放在人正看着的那个面板里。

## 版本历史

被智能体反复重写的产物会留下它替换掉的内容。打开一份写过不止一次的产物，标题旁会出现「历史」按钮；只写过一次的产物——大多数都是——看不到任何与历史有关的东西。
...
版本存在产物旁边的 `.versions/` 下，每份产物保留最近 20 个（`maxVersionsPerArtifact`）。内容没变的重写不算一个版本。删除产物会连同它的历史一起删掉。
